...
public class DefaultBatchLoader implements BatchLoader<DataFetchingEnvironment, DataFetcherResult<Object>>  {
  ...
  private final FieldRequestRequestFactory fieldRequestRequestFactory;
  ...
  public CompletionStage<List<DataFetcherResult<Object>>> load(List<DataFetchingEnvironment> environments) {
    ...
    for (DataFetchingEnvironment dataFetchingEnvironment : environments) {
        subGraphQueryPlan.add(fieldRequestRequestFactory.createFrom(dataFetchingEnvironment));
    }
